...
def processWithNumpy(signal):
   ...
   plotarr(transformedSignal,0)
   frequencyFilter(transformedSignal)
   plotarr(transformedSignal,0)
   
   cleanedSignal = numpy.fft.ifft(transformedSignal)
   #print cleanedSignal[20000:20010]
   #plotarr(cleanedSignal)
...
def plotarr(arr,samplingRate):
   samplingRate = 22050
   arr_len = len(arr)
   #plt.ion()
   plt.plot(map(lambda x:samplingRate*x/arr_len,range(arr_len)),[norm(x) for x in arr])
